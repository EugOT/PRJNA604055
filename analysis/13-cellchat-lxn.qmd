---
title: "CellChat signalling analysis of Arcuate Nucleus/Median Eminence Lxn (+/-) astrocytes dataset from Deng G et al 2020"
...
DOCNAME <- "cellchat-astrocytes-between-conditions-astrocytes-dataset-from-deng2020"
NOW <- Sys.time()
...
```{r filter-cells}
Idents(srt, cells = WhichCells(srt, expression = (Pomc > 20 & Rbfox3 > 10 & ora_celltype == "Neurons"))) <- "POMC neurons"
Idents(srt, cells = WhichCells(srt, expression = ((Agrp > 5 | Npy > 5) & Rbfox3 > 5 & ora_celltype == "Neurons"))) <- "AGRP/NPY neurons"
Idents(srt, cells = WhichCells(srt, expression = (Lxn > 0 & Rbfox3 < 5 & cell_name %in% astrocytes_idx))) <- "Astro_LXN+"
Idents(srt, cells = WhichCells(srt, expression = (Lxn == 0 & Rbfox3 < 5 & cell_name %in% astrocytes_idx))) <- "Astro_LXN-"
...
if (file.exists(here(data_dir, "cellchat-deng2020-HFD_and_control-lxn.rds"))) {
    object.list <- readr::read_rds(file = here(data_dir, "cellchat-deng2020-HFD_and_control-lxn.rds"))
} else {
  # set the used database in the object
  object.list$hfd@DB <- CellChatDB.use
  ...
  readr::write_rds(object.list, file = here(data_dir, "cellchat-deng2020-HFD_and_control-lxn.rds"))
}
```

## Plot CellChat network of HFD and control mice
...
```{r plot-cellchat-net-hfd}
cellchat <- object.list$hfd
groupSize <- as.numeric(table(cellchat@idents))
par(mfrow = c(1, 2), xpd = TRUE)
netVisual_circle(cellchat@net$count, vertex.weight = groupSize, weight.scale = T, label.edge = T, title.name = "Number of interactions")
netVisual_circle(cellchat@net$weight, vertex.weight = groupSize, weight.scale = T, label.edge = T, title.name = "Interaction weights/strength")
...
```{r plot-cellchat-net-per-group-hfd}
mat <- cellchat@net$weight
par(mfrow = c(2, 2), xpd = TRUE)
for (i in 1:nrow(mat)) {
  mat2 <- matrix(0, nrow = nrow(mat), ncol = ncol(mat), dimnames = dimnames(mat))
...
## Compare CellChat network of HFD and control mice

### Compare the total number of interactions and interaction strength

```{r plot-cellchat-net-compare, fig.width = 4, fig.height = 3}
object.list <- list(control = object.list$control, HFD = object.list$hfd)
...
This bar graph can be plotted in a stacked mode or not. Significant signaling pathways were ranked based on differences in the overall information flow within the inferred networks between HFD and control mice arcuate nucleus of hypothalamus. The top signaling pathways colored red are enriched in HFD mice, and these colored blue were enriched in the control mice.
...
# extract the ligand-receptor pairs with upregulated ligands in HFD
net.up <- subsetCommunication(cellchat, net = net, datasets = "HFD", ligand.logFC = 0.2, receptor.logFC = NULL)
# extract the ligand-receptor pairs with upregulated ligands and upregulated recetptors in control, i.e.,downregulated in HFD
...
pathways.show.all <- intersect(object.list$HFD@netP$pathways, object.list$control@netP$pathways)
...
cellchat@meta$datasets <- factor(cellchat@meta$datasets, levels = c("HFD", "control")) # set factor level
for (i in 1:length(pathways.show.all)) {
  tryCatch(
    {
        pathways.show <- pathways.show.all[i]
...
readr::write_rds(cellchat, file = here(data_dir, "cellchat-comparison-deng2020-HFD_vs_control-lxn.rds"))
```
