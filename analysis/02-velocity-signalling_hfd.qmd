...
adata_norm.raw.X[:10, :10].todense()
```
...
pomc_neurons = adata_norm[
    (
        (adata_norm[:, "Pomc"].to_df() > 20).stack().values
        & (adata_norm[:, "Rbfox3"].to_df() > 10).stack().values
    )
...
npy_neurons = adata_norm[
    (
        (
            (adata_norm[:, "Agrp"].to_df() > 5).stack().values
            | (adata_norm[:, "Npy"].to_df() > 5).stack().values
        )
...
sc.pl.embedding(
    ...
    palette="nipy_spectral",
    show=True,
    save="-pacmap-adgene-HF-diet.pdf",
)
```
...
with matplotlib.rc_context({"figure.figsize": (5.5, 4)}):
    sc.pl.violin(
        ...
        palette="nipy_spectral",
        show=True,
        save="-violin-adgene-HF-diet.pdf",
    )
```
...
pomc_neurons = adata_hfd[
    (
        (adata_hfd[:, "Pomc"].to_df() > 20).stack().values
        & (adata_hfd[:, "Rbfox3"].to_df() > 10).stack().values
    )
...
npy_neurons = adata_hfd[
    (
        (
            (adata_hfd[:, "Agrp"].to_df() > 5).stack().values
            | (adata_hfd[:, "Npy"].to_df() > 5).stack().values
        )
...
sc.pl.embedding(
    ...
    palette="nipy_spectral",
    show=True,
    save="-pacmap-subset-int-HF-diet.pdf",
)
```
...
sc.pl.embedding(
    ...
    palette="nipy_spectral",
    show=True,
    save="-pacmap-subset-adgene-HF-diet.pdf",
)
```
...
n2a_hfd.save(filename="liana-dotplot-logfc-HF-diet-n2a.pdf")
```
...
n2a_hfd.save(filename="liana-dotplot-specificity_rank-HF-diet-n2a.pdf")
```
...
n2a_hfd.save(filename="liana-dotplot-magnitude_rank-HF-diet-n2a.pdf")
```
...
a2n_hfd.save(filename="liana-dotplot-logfc-HF-diet-a2n.pdf")
```
...
a2n_hfd.save(filename="liana-dotplot-specificity_rank-HF-diet-a2n.pdf")
```
...
a2n_hfd.save(filename="liana-dotplot-magnitude_rank-HF-diet-a2n.pdf")
```
...
```{python}
adata1 = scv.read("../piscem_spliceu/SRR10992871/SRR10992871.h5ad")
adata2 = scv.read("../piscem_spliceu/SRR10992872/SRR10992872.h5ad")
adata3 = scv.read("../piscem_spliceu/SRR10992873/SRR10992873.h5ad")
adata4 = scv.read("../piscem_spliceu/SRR10992874/SRR10992874.h5ad")
adata5 = scv.read("../piscem_spliceu/SRR10992875/SRR10992875.h5ad")
adata6 = scv.read("../piscem_spliceu/SRR10992876/SRR10992876.h5ad")

# prepare cell names
cells1 = "SRR10992871_" + adata1.obs_names + "-1"
adata1.obs_names = cells1
adata1.obs["run"] = "SRR10992871"
cells2 = "SRR10992872_" + adata2.obs_names + "-1"
adata2.obs_names = cells2
adata2.obs["run"] = "SRR10992872"
cells3 = "SRR10992873_" + adata3.obs_names + "-1"
adata3.obs_names = cells3
adata3.obs["run"] = "SRR10992873"
cells4 = "SRR10992874_" + adata4.obs_names + "-1"
adata4.obs_names = cells4
adata4.obs["run"] = "SRR10992874"
cells5 = "SRR10992875_" + adata5.obs_names + "-1"
adata5.obs_names = cells5
adata5.obs["run"] = "SRR10992875"
cells6 = "SRR10992876_" + adata6.obs_names + "-1"
adata6.obs_names = cells6
adata6.obs["run"] = "SRR10992876"
...
adata4 = adata4[adata4.obs_names.isin(adata.obs_names), :]
adata5 = adata5[adata5.obs_names.isin(adata.obs_names), :]
adata6 = adata6[adata6.obs_names.isin(adata.obs_names), :]
velocity_data = ad.concat(
    [adata1, adata2, adata3, adata4, adata5, adata6], join="inner"
)
```
...
```{python}
scv.pl.proportions(velocity_data, groupby="libname", save="-libname-proportions.pdf")
```
...
sc.tl.paga(velocity_data, groups="libname")
...
```{python}
save_PRJNA604055 = f"-PRJNA604055-astrocytes_datasets-velocity.pdf"
...
scv.pl.velocity_embedding_stream(
    ...
    show=True,
    figsize=plt_size,
    save="umap-libname" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    show=True,
    figsize=plt_size,
    save="pacmap-stream" + save_PRJNA604055,
)
```

```{python}
sc.tl.embedding_density(velocity_data, basis="pacmap", groupby="libname")
sc.pl.embedding_density(
    velocity_data,
    basis="pacmap",
    key="pacmap_density_libname",
    show=True,
    save="pacmap-libname" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="-npep-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="-npr-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="-nmr-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="-adgen-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="-mitochondrial-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="pacmap-npep-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="pacmap-npr-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="pacmap-nmr-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="pacmap-adgen-stream" + save_PRJNA604055,
)
```
...
scv.pl.velocity_embedding_stream(
    ...
    size=40,
    ncols=4,
    save="pacmap-mitochondrial-stream" + save_PRJNA604055,
)
```
...
cr.tl.terminal_states(
    velocity_data, cluster_key="k_tree", weight_connectivities=0.2, softmax_scale=4
)
cr.tl.initial_states(velocity_data, cluster_key="k_tree")
cr.tl.lineages(velocity_data)
cr.pl.lineages(velocity_data, same_plot=True)
```
...
scv.pl.scatter(
    ...
    color_map="gnuplot",
    size=80,
    save="umap-latent-time" + save_PRJNA604055,
)
scv.pl.scatter(
    velocity_data,
    basis="pacmap",
    ...
    color_map="gnuplot",
    size=80,
    save="pacmap-latent-time" + save_PRJNA604055,
)
```
...
```{python}
scv.pl.scatter(velocity_data, genes_embed[40:], frameon=False)
scv.pl.scatter(velocity_data, x="latent_time", y=genes_embed[40:], frameon=False)
```
...
scv.pl.velocity_embedding(
    ...
    dpi=120,
    show=True,
    save="embedding" + save_PRJNA604055,
)
```
...
scv.pl.scatter(
    ...
    cmap="coolwarm",
    perc=[5, 95],
    save="umap-velocity_confidence" + save_PRJNA604055,
)
scv.pl.scatter(
    velocity_data,
    basis="pacmap",
    ...
    cmap="coolwarm",
    perc=[5, 95],
    save="pacmap-velocity_confidence" + save_PRJNA604055,
)
```
...
```{python}
cr.tl.lineage_drivers(velocity_data)
```


```{python}
model = cr.ul.models.GAM(velocity_data)

cr.pl.gene_trends(
    velocity_data,
    model=model,
    data_key="X",
    genes=[g for g in genes_embed if g in velocity_data.var_names],
    ncols=3,
    ...
    figsize=(12, 12),
    n_test_points=200,
    save="-adgen-gene_trends" + save_PRJNA604055,
)
```
...
cr.pl.gene_trends(
    ...
    model=model,
    data_key="X",
    genes=[g for g in npep if g in velocity_data.var_names],
    ncols=3,
    time_key="latent_time",
    same_plot=True,
    hide_cells=True,
    figsize=(8, 4),
    n_test_points=200,
    save="-npep-gene_trends" + save_PRJNA604055,
)
```
...
cr.pl.gene_trends(
    ...
    model=model,
    data_key="X",
    genes=[g for g in npr if g in velocity_data.var_names],
    ncols=3,
    time_key="latent_time",
    same_plot=True,
    hide_cells=True,
    figsize=(8, 4),
    n_test_points=200,
    save="-npr-gene_trends" + save_PRJNA604055,
)
```
...
cr.pl.gene_trends(
    ...
    model=model,
    data_key="X",
    genes=[g for g in nmr if g in velocity_data.var_names],
    ncols=3,
    time_key="latent_time",
    same_plot=True,
    hide_cells=True,
    figsize=(12, 18),
    n_test_points=200,
    save="-nmr-gene_trends" + save_PRJNA604055,
)
```
...
cr.pl.gene_trends(
    ...
    model=model,
    data_key="X",
    genes=[g for g in mitochondrial if g in velocity_data.var_names],
    ncols=3,
    time_key="latent_time",
    same_plot=True,
    hide_cells=True,
    figsize=(12, 18),
    n_test_points=200,
    save="-mitochondrial-gene_trends" + save_PRJNA604055,
)
```
...
cr.pl.lineage_drivers(
    ...
    lineage=13,
    n_genes=12,
    save="umap-lineage-drivers" + save_PRJNA604055,
)
```
...
cr.pl.lineage_drivers(
    ...
    lineage=13,
    n_genes=12,
    save="pacmap-lineage-drivers" + save_PRJNA604055,
)
```
...
cr.pl.initial_states(
    velocity_data,
    basis="umap",
    save="umap-initial-states" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.initial_states(
    velocity_data,
    basis="pacmap",
    save="pacmap-initial-states" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.terminal_states(
    velocity_data,
    basis="pacmap",
    save="pacmap-terminal-states" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.terminal_states(
    velocity_data,
    basis="umap",
    save="umap-terminal-states" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.lineages(
    ...
    basis="umap",
    same_plot=False,
    save="umap-lineages" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.lineages(
    ...
    basis="pacmap",
    same_plot=False,
    save="pacmap-lineages" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.lineages(
    ...
    basis="umap",
    same_plot=True,
    save="umap-lineages-same" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.lineages(
    ...
    basis="pacmap",
    same_plot=True,
    save="pacmap-lineages-same" + save_PRJNA604055,
)
plt.show()
```
...
cr.pl.heatmap(
    velocity_data,
    model,
    genes=velocity_data.varm["terminal_lineage_drivers"]["13_corr"]
    ...
    n_jobs=threads,
    backend="loky",
    save="-terminal_0-heatmap" + save_PRJNA604055,
)
```
